...
// filterBy
// Array.prototype.filterBy = function <k>(
//   prop: keyof T,
...
Array.prototype.filterBy = function (prop, value, isIncludes = false) {
  return this.filter((a) =>
    isIncludes ? a[prop]?.isIncludes(value) : a[prop] === value
  );
...
console.log(users.groupByFn(({ dept }) => dept)); // Objec.groupBy(users, cb)
...
console.log(users.groupBy("dept")); // Objec.groupBy(users, cb)
